          ...
          if (_(opts.include).isUndefined()) {
            if (_(selector).isArray())
              ...
              if (_.isUndefined(result))
                results = empty
              else if (rowOrRows && rowOrRows._count && _.size(rowOrRows) == 1)
                results = { count: parseInt(rowOrRows._count) }
              else
                results = rowOrRows
            }
...
Base.prototype.update = function(selector, obj, opts, callback) {
  var self = this;
    ...
    var updateQuery = function() {
        var outValues = [];
    var updateStatement =
            self.client.statements.update(self, selector, obj, opts, outValues);
