...
exports.update = function(model, selector, obj, opts, outValues) {
  var stmt  = "UPDATE " + '"' + model.tableName + '"'
        , set   = buildUpdateFields(model, obj, outValues)
        , where = buildWhereClause(model, selector, outValues)
        , returning = buildReturnClause(opts);
    return stmt + set + where + returning + ';';
};
...
var buildReturnClause = function(opts) {
    if (_(opts.returning).isUndefined()) {
        return "";
    } else {
...
var buildOperator = function(model, key, value, outValues) {
  ...
  if(value === null || value === void 0 || value === '$null') {
    // Instead of $null, we can use value as an object with explicit null type
    if(keyOp == 'ne') {
  ...
  // Fixes issues when using array types in Postgres
